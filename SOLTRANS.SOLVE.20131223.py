...
while yr < t_yr:
  ...
  while t < t_dy:
    ...
    Ml *= dt / (R[t,1:] * teta_[t,1:]) 
    
    # main diagonal: coefficients of c[i]
    #diffusion
    Mm =  (D[t, 1:-2] + D[t, 2:-1]) / dz**2
    #advection
    Mm += (np.maximum(0, fw[t, 2:-1]) - np.minimum(0, fw[t, 1:-2])) / dz
    #root uptake
    Mm += r_uptk[t, 1:-1] 
    #scaling operator (retardation, 2nd derivative dz, time step)
    Mm *= dt / (R[t, 1:-1] * teta_[t, 1:-1])
    #outside scaling factor because radiodecay of both liquid & sorbed phases
    Mm += dt * dcy[ele]    
    Mm += 1.
    ...
    #every soil segment)    
    M = np.vstack((M, -r_uptk[t, :] * dt/ (R[t, :] * teta_[t, :])))
    ...
    # top boundary
    #inputs with infiltration of rainwater & irrigations    
    S[0] = (switch['off']*np.maximum(0, fw[t, 0]) + irr[t])/dz * Cgw  
    ...
    if s_base_flux == 'n':
      S[-2] = - np.minimum(0, fw[t,-1])/dz * Cgw * switch['off']
    else:    
      S[-2] = - np.minimum(0, fw[t,-1])/dz * Cgw * switch['on']
      
    S[-2] *= dt / (R[t,-1] * teta_[t, -1])    
