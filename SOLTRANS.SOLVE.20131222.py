...
s_sur_flux = (raw_input("apply irrigation? [y/n]: "))
if type(s_sur_flux) is not str:
  print 'not a valid string!'
  sys.exit()
  
s_base_flux = (raw_input("allow bottom influx? [y/n]: "))
if type(s_sur_flux) is not str:
  print 'not a valid string, execusion terminated!'
  sys.exit()
...
while yr < t_yr:
  ...
  while t < t_dy:
    ...
    S[0] = np.maximum(0, fw[t, 0])/dz * Cgw * switch['off'] 
    #then add inputs with irrigations    
    S[0] += irr[t]/dz * Cgw * switch['on']
